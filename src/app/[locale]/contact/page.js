...
export async function generateMetadata({ params }) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}contact-pages/`);
  ...
  return {
    ...
    openGraph: {
      ...
      description: data.seo_description,
      images: [data.image],
      url: `${baseUrl}${locale === "en" ? "" : locale}/contact/`,
      type: 'website',
      siteName: 'WMX School',
    },
    alternates: {
...
export default async function Page({ params }) {
  ...
  const [
    ...
    fetchData(`contact-pages/`, locale)
  ]);
  ...
  //         answer:
  //           "You'll receive email notifications contat updastes and can download the latest version from your account dashboard at any time.",
  //       },
  //     ],
  //   },
  // };
